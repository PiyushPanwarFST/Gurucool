...
import Bull from "bull"; // Bull for managing message queues
...
authQueue.add({ message: "Job data" });
...
const login = async (req, res) => {
  try {
    ...
    if (!process.env.SECRET_KEY) {
      return res
        .status(500)
        .send({ message: "Missing SECRET_KEY environment variable!" });
    }

    // Generate a JWT token containing the client ID as payload
    const token = jwt.sign({ clientId: client._id }, process.env.SECRET_KEY);
